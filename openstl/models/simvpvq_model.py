...
from openstl.modules import Quantization


class SimVPVQ_Model(nn.Module):

    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA',
                 mlp_ratio=8., drop=0.0, drop_path=0.0, spatio_kernel_enc=3,
                 spatio_kernel_dec=3, act_inplace=False, 
                 post_vq=False, vq_type='svq', vq_kwargs=None, noise=None,
                 **kwargs):
        super(SimVPVQ_Model, self).__init__()
        T, C, H, W = in_shape  # T is pre_seq_length
        self.T = T
        act_inplace=False
        
        self.enc = Encoder(C, hid_S, N_S, spatio_kernel_enc, act_inplace)
        self.dec = Decoder(H*2**(N_S/2), hid_S, C, N_S, spatio_kernel_dec, act_inplace)
        self.post_vq = post_vq
        self.vq = Quantization(vq_type, vq_kwargs)
    ...
    def forward(self, x_raw,  y_raw=None, **kwargs):
        if self.noise and self.training:
            x_raw = self.perturb(x_raw)
        
        B, T, C, H, W = x_raw.shape
        x = x_raw.view(B*T, C, H, W)
        latent, skip = self.enc(x)
        B_, C_, H_, W_ = latent.shape
        
        perplexity_all = dict()
        
        if self.post_vq:
            hid = self.hid(latent.view(B, T, C_, H_, W_)).reshape(B*T, C_, H_, W_)
            continuous = hid
            quantize, auxilary_loss, perplexity, code_weight, codebook = self.vq(continuous)
            hid = quantize
        else:
            continuous = latent
            quantize, auxilary_loss, perplexity, code_weight, codebook = self.vq(continuous)
            hid = quantize.view(B, T, C_, H_, W_)
            hid = self.hid(hid).reshape(B*T, C_, H_, W_)

        for perp_name in perplexity:
            perplexity_all[perp_name] = perplexity[perp_name]

        Y = self.dec(hid, skip)
        Y = Y.reshape(x_raw.shape)
        return Y, auxilary_loss, perplexity_all, continuous, quantize, code_weight, codebook

    def perturb(self, x):
        device = x.device
        data_num = x.nelement()
        noise_num = int(data_num * self.noise) #perturb_ratio
        noise = torch.rand(noise_num).to(device) * 4 -2
        mask = torch.cat((torch.ones(data_num-noise_num).to(device),noise))
...
class Encoder(nn.Module):
    ...
    def __init__(self, C_in, C_hid, N_S, spatio_kernel, act_inplace):
...
class Decoder(nn.Module):
    ...
    def __init__(self, H, C_hid, C_out, N_S, spatio_kernel, act_inplace):
