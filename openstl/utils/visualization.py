...
def show_video_error_line(data, ncols, vmax=0.6, cbar=False, format='png', out_path=None):
    """generate images with a video sequence"""
    ...
    if ncols == 1:
        sns.heatmap(data[0], cmap='GnBu', ax=axes, vmax=vmax, cbar=cbar)
        axes.axis('off')
    else:
        for t, ax in enumerate(axes.flat):
            # im = sns.heatmap(data[t], cmap='GnBu', ax=ax, vmax=vmax, cbar=cbar)
            im = ax.imshow(data[t], cmap='GnBu')
            ax.axis('off')
            im.set_clim(0, vmax)
