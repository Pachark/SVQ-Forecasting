...
class BaseExperiment(object):
    ...
    def __init__(self, args, dataloaders=None):
        ...
        self.early_stopper = EarlyStopper(patience=self.args.patience)
    ...
    def _acquire_device(self):
        ...
        if self.args.use_gpu:
            ...
            if self.args.dist:
                ...
                print_log('Use non-distributed mode with GPU:' + str(device))
        else:
            self._use_gpu = False
    ...
    def _preparation(self, dataloaders=None):
        ...
        if self.args.ex_name != 'Debug':
            ex_path = self.args.ex_name if not self.args.ex_name.startswith(self.args.res_dir) \
                else self.args.ex_name.split(self.args.res_dir+'/')[-1]
        else:
            config_keys = ['model_type', 'post_vq', 'vq_type', 'vq_kwargs',
                          'loss', 'nonlinear', 'middim','lista_iter', 'freeze_projection', 'stochastic_sample_codes', 'noise']
            ex_path = '_'.join([k+str(self.config[k]) for k in self.config.keys() if k in config_keys])

        self.path = osp.join(base_dir, ex_path)
    ...
    def display_method_info(self):
        ...
        if self.args.method in ['simvp', 'tau', 'simvpvq']:
        ...
        elif self.args.method in ['phydnet']:
    ...
    def train(self):
        ...
        for epoch in range(self._epoch, self._max_epochs):
            ...
            intermediate.append([epoch + 1, loss_mean.avg, vali_loss, val_mse])
            intermediate_cols = ['epoch', 'train_loss', 'val_loss', 'val_mse']
            try:
                for p in self.method.perplexity_train:
                    intermediate[-1] = intermediate[-1] + [self.method.perplexity_train[p].item()]
                    intermediate_cols = intermediate_cols + [p]
            except:
                pass
            
            if self.early_stopper.early_stop(vali_loss):             
    ...
    def vali(self):
        ...
        return results['loss'].mean(), results['mse'].mean()
    ...
    def test(self):
        ...
        results['metrics'] = np.array([eval_res[i] for i in eval_res.keys()])
