...
class encoder_E_VQ(nn.Module):

    def __init__(self, nc=1, nf=32, patch_size=4):
        super(encoder_E_VQ, self).__init__()
...
class decoder_D_VQ(nn.Module):

    def __init__(self, nc=1, nf=32, patch_size=4):
        super(decoder_D_VQ, self).__init__()
        assert patch_size in [2, 4]
        stride_2 = patch_size // 2
        ...
        self.vq1 = STVQ(dim=2*nf)
        self.vq2 = STVQ(dim=nf)
        self.vq3 = STVQ(dim=nf)
        self.merge1 = nn.Conv2d(2*nf, nf, 1)
        self.merge2 = nn.Conv2d(2*nf, nf, 1)
    ...
    def forward(self, input, res):
        ...
        d2 = self.merge1(torch.cat([d2,res[-1]], axis=1))
        d2vq = self.vq2(d2)
        d3 = self.upc2(d2vq)
        
        d3 = self.merge2(torch.cat([d3,res[-2]], axis=1))
        d3vq = self.vq3(d3)
        d4 = self.upc3(d3vq)
        return d4
...
#         self.c2 = dcgan_conv(nf, nf, stride=1) # (32) x 32 x 32
#         self.c3 = dcgan_conv(nf, 2*nf, stride=stride_2) # (64) x 16 x 16
#         self.vq1 = STVQ(dim=32)
#         self.vq2 = STVQ(dim=32)
#         self.vq3 = STVQ(dim=64)
...
#         stride_2 = patch_size // 2
#         output_padding = 1 if stride_2==2 else 0
#         self.upc1 = dcgan_upconv(4*nf, nf, stride=2) #(32) x 32 x 32
#         self.upc2 = dcgan_upconv(2*nf, nf, stride=1) #(32) x 32 x 32
#         self.upc3 = nn.ConvTranspose2d(in_channels=2*nf, out_channels=nc, kernel_size=(3,3),
#                                        stride=stride_2, padding=1,
#                                        output_padding=output_padding)  #(nc) x 64 x 64
...
#         # import pdb
#         # pdb.set_trace()
#         d1 = self.upc1(torch.cat([input,res[-1]], axis=1))
#         d2 = self.upc2(torch.cat([d1,res[-2]], axis=1))
#         d3 = self.upc3(torch.cat([d2,res[-3]], axis=1))
#         return d3
    
    
class PhyD_EncoderRNN_VQ(torch.nn.Module):

    def __init__(self, phycell, convcell, in_channel=1, patch_size=4):
        super(PhyD_EncoderRNN_VQ, self).__init__()
        self.encoder_E = encoder_E_VQ(nc=in_channel, patch_size=patch_size) # general encoder 64x64x1 -> 32x32x32
        ...
        self.decoder_D = decoder_D_VQ(nc=in_channel, patch_size=patch_size) # general decoder 32x32x32 -> 64x64x1

        self.phycell = phycell
        self.convcell = convcell

    def forward(self, input, first_timestep=False, decoding=False):
        input, res = self.encoder_E(input) # general encoder 64x64x1 -> 32x32x32
        ...
        out_phys = torch.sigmoid(self.decoder_D(decoded_Dp, res)) # partial reconstructions for vizualization
        out_conv = torch.sigmoid(self.decoder_D(decoded_Dr, res))

        concat = decoded_Dp + decoded_Dr   
        output_image = torch.sigmoid( self.decoder_D(concat, res))
