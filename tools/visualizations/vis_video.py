...
from openstl.utils import (show_video_gif_multiple, show_video_gif_single, show_video_line,
                           show_taxibj, show_weather_bench, show_video_error_line)
...
def main():
    ...
    use_rgb = False if args.dataname in ['mfmnist', 'mmnist', 'kth20', 'kth', 'kth40', 'era'] else True
    ...
    for method in method_list:
        if 'human' in args.dataname:
            inputs = np.load(os.path.join(base_dir, method_list[0], 'saved/trues.npy'))
        else:
    ...
    for i, method in enumerate(method_list):
        ...
        errors = np.load(os.path.join(base_dir, method, 'saved/trues.npy')) - np.load(os.path.join(base_dir, method, 'saved/preds.npy'))
        errors = errors[idx, 0:ncols, ...]
        ...
        if 'weather' in args.dataname:
            ...
            errors = errors[:,args.vis_channel,...]
        elif 'taxibj' in args.dataname:
        ...
        cbar = False
        if i == 0:
            show_video_line(inputs.copy(), ncols=config['pre_seq_length'], vmax=0.6, cbar=cbar,
                out_path='{}/{}_input{}'.format(args.save_dirs, args.dataname+c_surfix, str(idx)+'.png'),
                format='png', use_rgb=use_rgb)
            show_video_line(trues.copy(), ncols=config['aft_seq_length'], vmax=0.6, cbar=cbar,
        ...
        show_video_line(preds, ncols=ncols, vmax=0.6, cbar=cbar,
                        out_path='{}/{}_{}_{}'.format(args.save_dirs, args.dataname+c_surfix, method, str(idx)+'.png'),
                        format='png', use_rgb=use_rgb)
        if args.dataname == 'taxibj' or 'weather' in args.dataname:
            show_video_error_line(errors, ncols=ncols, vmax=0.3, cbar=cbar,
                            out_path='{}/{}_error_{}_{}'.format(args.save_dirs, args.dataname+c_surfix, method, str(idx)+'.png'),
                            format='png')
